...
definition cshift :: "bool list \<Rightarrow> coin_stream \<Rightarrow> coin_stream" where 
  "cshift x y = to_coins (x @- from_coins y)"

definition "cnth x n = from_coins x !! n"
definition "ctake n x = stake n (from_coins x)"
definition "cdrop n x = to_coins (sdrop n (from_coins x))"
definition "rel_coins x y = (to_coins x = y)"
definition "cmap f x = to_coins (smap f (from_coins x))"
definition "cprefix x y \<longleftrightarrow> ctake (length x) y = x" 
...
lemma cmap_transfer [transfer_rule]: "((=) ===> rel_coins ===> rel_coins) smap cmap"
  unfolding rel_coins_def cmap_def rel_fun_def by (auto simp:from_to_coins)
...
lemma lenth_ctake [simp]: "length (ctake n x) = n"
...
proof -
  ...
  proof -
    have "{xs. cnth xs i = v} = (\<lambda>xs. cnth xs i) -` {v} \<inter> space \<B>"
      unfolding vimage_def space_coin_space by auto
    also have "... \<in> sets \<B>"
      by (intro measurable_sets[OF cnth_measurable]) auto
    finally show ?thesis
      by simp
  qed
